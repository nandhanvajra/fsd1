...
mongoose.connect('mongodb://localhost:27017/todo-app').then(console.log("connected to db"))
...
app.put('/:id',authenticate,async(req,res)=>{
   const {title,description,status,dueDate,assignedTo}=req.body;
    try{
        ...
        const task=await Task.findByIdAndUpdate(req.params.id,{title,description,status,dueDate,assignedTo},
            {
                new:true,
                runValidators:true
            }
...
app.post('/signup',async(req,res)=>{
   ...
   const user=await User.findOne({email});
   if(user){
      res.status(409).send({message:'This user allready exists'})
   }
   else{
